...
<?php echo esc_html($_GET['ln_auth_err']);?>
 &nbsp;&nbsp;&nbsp;<span
		id="system_notice_area_dismiss" class="xyz_lnap_hide_ln_authErr">Dismiss</span>
</div>
...
// 			print_r($_POST);//xyz_lnap_ln_share_post_company
	...
	{
		$lms3="	Please select share post to profile (or company)";
		$lerf=1;
	}
...
<div class="xyz_lnap_tab">
  <button class="xyz_lnap_tablinks" onclick="xyz_lnap_open_tab(event, 'xyz_lnap_linkedin_settings')" id="xyz_lnap_default_tab_settings">linkedin Settings</button>
   <button class="xyz_lnap_tablinks" onclick="xyz_lnap_open_tab(event, 'xyz_lnap_basic_settings')" id="xyz_lnap_basic_tab_settings">General Settings</button>
</div>
	...
	?>
 	...
 	<span id="auth_message">
 		...
 		<input type="submit" class="submit_lnap_new" name="ln_auth"
 			value="Authorize" onclick="javascript:return lnap_popup_ln_auth('<?php echo urlencode($domain_name);?>','<?php echo $xyz_lnap_smapsoln_userid;?>','<?php echo $xyzscripts_user_id;?>','<?php echo $xyzscripts_hash_val;?>','<?php echo $auth_secret_key;?>','<?php echo $request_hash;?>');void(0);"/><br><br>
 		</form> 
 		</span>
 		<?php }
 		else if($lnaf==0 )
 		...
 		<input type="hidden" value="<?php echo  (is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST']; ?>" id="parent_domain">
 		<input type="submit" class="submit_lnap_new" name="ln_auth"
 		value="Reauthorize" title="Reauthorize the account" onclick="javascript:return lnap_popup_ln_auth('<?php echo urlencode($domain_name);?>','<?php echo $xyz_lnap_smapsoln_userid;?>','<?php echo $xyzscripts_user_id;?>','<?php echo $xyzscripts_hash_val;?>','<?php echo $auth_secret_key;?>','<?php echo $request_hash;?>');void(0);"/><br><br>
 		</form>
 	<?php }
}?>
...
<?php wp_nonce_field( 'xyz_lnap_ln_settings_nonce' );?>
	...
	<a href="javascript:lnap_popup_connect_to_xyzscripts();void(0);" style="color:white !important;">Connect your xyzscripts account</a>
	</span>
	</td>
	</tr>
	<?php }?>
	...
	<tr valign="top">
		...
		<select id="xyz_lnap_lnpost_method" name="xyz_lnap_lnpost_method">
				<option value="1"
	<?php  if(get_option('xyz_lnap_lnpost_method')==1) echo 'selected';?>>Simple text message</option>
				<option value="2"
	<?php  if(get_option('xyz_lnap_lnpost_method')==2) echo 'selected';?>>Attach your blog post </option>
		</select>
		</td>
	</tr>
	<tr valign="top">
	<td>Enforce og tags for LinkedIn<img src="<?php echo $heimg?>" onmouseover="detdisplay_lnap('xyz_lnap_enforce_og')" onmouseout="dethide_lnap('xyz_lnap_enforce_og')" style="width:13px;height:auto;">
	<div id="xyz_lnap_enforce_og" class="lnap_informationdiv" style="display: none;width: 400px;">
	If you enable, Open Graph tags will be generated while posting to LinkedIn, when using the posting method <b>Attach your blog post</b>.
	</div>
	</td>
	<td  class="switch-field">
		<label id="xyz_lnap_enforce_og_tags_yes" class="xyz_lnap_toggle_off"><input type="radio" name="xyz_lnap_enforce_og_tags" value="1" <?php  if(get_option('xyz_lnap_enforce_og_tags')==1) echo 'checked';?>/>Yes</label>
		<label id="xyz_lnap_enforce_og_tags_no" class="xyz_lnap_toggle_on"><input type="radio" name="xyz_lnap_enforce_og_tags" value="0" <?php  if(get_option('xyz_lnap_enforce_og_tags')==0) echo 'checked';?>/>No</label>
    </td>
	</tr> 
	<tr valign="top">
...
<?php if (get_option('xyz_lnap_ln_api_permission')==2 && $lnaf==0 ){
	//////////////////////////////////////////////
	?>
	<tr valign="top"><td>Share post to profile </td>
	<td  class="switch-field">
	<?php  if(get_option('xyz_lnap_lnshare_to_profile')==0){?>
		<label id="xyz_lnap_lnshare_to_profile_smap_yes" class="xyz_lnap_toggle_off"><input type="radio" name="xyz_lnap_lnshare_to_profile_smap" value="1" disabled/>Yes</label>
		<label id="xyz_lnap_lnshare_to_profile_lnap_no" class="xyz_lnap_toggle_on"><input type="radio" name="xyz_lnap_lnshare_to_profile_smap" value="0" checked/>No</label>
		<?php }
		elseif(get_option('xyz_lnap_lnshare_to_profile')==1){
			?>
		<label id="xyz_smap_lnshare_to_profile_smap_yes" class="xyz_lnap_toggle_on"><input type="radio" name="xyz_lnap_lnshare_to_profile_smap" value="1" checked/>Yes</label>
		<label id="xyz_lnap_lnshare_to_profile_smap_no" class="xyz_lnap_toggle_off"><input type="radio" name="xyz_lnap_lnshare_to_profile_smap" value="0" disabled/>No</label>
			<?php 
		}?>
	...
	<td  class="switch-field">
		<label id="xyz_lnap_lnshare_to_profile_yes" ><input type="radio" name="xyz_lnap_lnshare_to_profile" value="1" <?php  if(get_option('xyz_lnap_lnshare_to_profile')==1) echo 'checked';?>/>Yes</label>
		<label id="xyz_lnap_lnshare_to_profile_no" ><input type="radio" name="xyz_lnap_lnshare_to_profile" value="0" <?php  if(get_option('xyz_lnap_lnshare_to_profile')==0) echo 'checked';?>/>No</label>
	</td>
	</tr>
	<?php }?>
...
<?php wp_nonce_field( 'xyz_lnap_basic_settings_nonce' );?>
			...
			id="xyz_lnap_sel_cat"> 
				...
				<tr valign="top">
					...
					<td>
						<input type="radio" name="xyz_lnap_default_selection_edit" value="1" <?php  if($xyz_lnap_default_selection_edit==1) echo 'checked';?>/>Enabled
						<br/><input type="radio" name="xyz_lnap_default_selection_edit" value="0" <?php  if($xyz_lnap_default_selection_edit==0) echo 'checked';?>/>Disabled
						<br/><input type="radio" name="xyz_lnap_default_selection_edit" value="2" <?php  if($xyz_lnap_default_selection_edit==2) echo 'checked';?>/>Use settings from post creation or post updation
					</td>
				</tr>
